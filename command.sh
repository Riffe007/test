...
for label, var in [("PyTorch", "pt_metrics"), ("TFLite", "tfl_metrics")]:
    ...
    else: notes.append(f"○ {label} log already patched")
...
else: notes.append("○ ExecuTorch log already patched")
EVAL.write_text(src)
py_compile.compile(str(EVAL), doraise=True)
print("✓ Syntax OK")
...
gt = json.load(open('../dataset/voc2012_as_coco/instances_voc2012_val.json'))
print('categories defined:', len(gt['categories']))
print('annotations:', len(gt['annotations']))
print('cat_id usage:')
cnt = Counter(a['category_id'] for a in gt['annotations'])
name_of = {c['id']: c['name'] for c in gt['categories']}
for cid, n in sorted(cnt.items()): print(f'  id={cid:3d}  count={n:5d}  name={name_of.get(cid, \"?\")}')
print('sample annotation:', gt['annotations'][0])
"



PYTHONPATH=../model_sources/MobileNetV2/src/pytorch/pytorch-ssd python evaluation/mobilenetv2/evaluate.py --voc-root ~/.cache/kagglehub/datasets/watanabe2362/voctrainval-11may2012/versions/1/VOCdevkit/VOC2012 --gt-coco-json ../dataset/voc2012_as_coco/instances_voc2012_val.json --tflite-model ../model_sources/MobileNetV2/weights/model.tflite --pytorch-model ../model_sources/MobileNetV2/weights/mobile_net_v2_ssd.pth --model-dir tests/integration/outputs/mobile_net_v2_ssd/basemodel_workflow/models --results-dir ../output/eval_mobilenet_v2_ssd --generate-report --max-samples 25 2>&1 | tee ../output/eval_smoke3.log




rm executorch-toolkit/evaluation/mobilenetv2/evaluate.py.bak executorch-toolkit/evaluation/mobilenetv2/evaluate.py.bak.1778624754

find ~/Documents/projects/MetaExecuTorch /tmp -maxdepth 6 -name "*.etdump" 2>/dev/null
