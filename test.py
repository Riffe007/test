...
constants and drop them, producing a quantization-free ONNX model that
downstream tools (notably onnx2torch) can consume.

Why this exists
---------------
TFLite INT8 models translate into ONNX with QuantizeLinear and
DequantizeLinear nodes wrapping the quantized values. onnx2torch has no
converter registered for these ops and raises NotImplementedError on
the first one it sees. This pass replaces every Q/DQ pair with the
FP32 tensor it represents:

    fp32_value = scale * (int_value - zero_point)

The model's numerical behavior is preserved at INT8 fidelity, but the
graph contains only float ops afterwards.

Scope (deliberately narrow)
---------------------------
Handles the patterns tf2onnx produces for TFLite-INT8 input:
    ...
    2. Producer -> QuantizeLinear -> DequantizeLinear -> consumer
       (activation re-quantization between layers; chained patterns
        across consecutive layers are folded iteratively)

Does NOT handle mixed-precision QLinearConv / QLinearMatMul fused ops;
